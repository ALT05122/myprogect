/*jshint -W065 */
var constructionbuilders = function( rowIndex, container, label ){
	'use strict';
...
wp.customize.controlConstructor['construction-builders-repeater'] = wp.customize.Control.extend({
	ready: function(){
		'use strict';
		...
		// The current value set in Control Class (set in Construction_Builders::to_json() function)
	...
	setImageInRepeaterField: function( attachment ){
		...
		$targetDiv.find( '.construction-builders-image-attachment' ).html( '<img src="' + attachment.url + '">' ).hide().slideDown( 'slow' );
	...
	setFileInRepeaterField: function( attachment ){
		...
		$targetDiv.find( '.construction-builders-file-attachment' ).html( '<span class="file"><span class="dashicons dashicons-media-default"></span> ' + attachment.filename + '</span>' ).hide().slideDown( 'slow' );
	...
	removeImage: function( event ){
		...
		$targetDiv.find( '.construction-builders-image-attachment' ).slideUp( 'fast', function() {
	...
	removeFile: function( event ){
		...
		$targetDiv.find( '.construction-builders-file-attachment' ).slideUp( 'fast', function() {
			jQuery( this ).show().html( jQuery( this ).data( 'placeholder' ) );
		});
		$targetDiv.find( '.hidden-field' ).val( '' );
		$uploadButton.text( $uploadButton.data( 'label' ) );
	...
	addRow: function( data ){
		...
		var control       = this,
		    template      = control.repeaterTemplate(), // The template for the new row (defined on Construction_Builders_Control_Repeater::render_content() ).
		...
		if ( template ) {
			...
			newRow = new constructionbuilders(
				control.currentIndex,
    ...
    $(document).on( 'click', 'input[type="font"]', function(){
        ...
        if( ( ! $this.hasClass('ajax-running') ) && $this.siblings( '.font-awesome-list' ).length < 1 ){
            $.ajax({
				...
				url : ajaxurl,
				data: {
					action             : 'construction_builders_get_fontawesome_ajax',
					construction_builders_customize_nonce: construction_builders_customize.nonce
				},
				beforeSend: function(){
